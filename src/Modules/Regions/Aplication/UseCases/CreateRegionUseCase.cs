...
public class CreateRegionUseCase
{
    ...
    public CreateRegionUseCase(IRegionRepository repository, IRegionValidator validator)
    {
    ...
    public async Task ExecuteAsync(string name, string type, int countryId)
    {
        ...
        await _validator.ValidateNameAsync(nameVO, countryVO);

        var entity = Region.CreateNew(nameVO, typeVO, countryVO);
